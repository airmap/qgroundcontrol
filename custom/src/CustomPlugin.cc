...
//-----------------------------------------------------------------------------
static QObject *customQuickInterfaceSingletonFactory(QQmlEngine *,
                                                     QJSEngine *) {
...
void CustomPlugin::setToolbox(QGCToolbox *toolbox) {
  ...
  qmlRegisterSingletonType<CustomQuickInterface>(
      "CustomQuickInterface", 1, 0, "CustomQuickInterface",
  ...
  connect(qgcApp()->toolbox()->corePlugin(),
          &QGCCorePlugin::showAdvancedUIChanged, this,
...
//-----------------------------------------------------------------------------
VideoManager *CustomPlugin::createVideoManager(QGCApplication *app,
                                               QGCToolbox *toolbox) {
...
void CustomPlugin::createRootWindow(QQmlApplicationEngine *qmlEngine) {
  ...
  qmlEngine->addImportPath("qrc:/Custom/Camera");
  qmlEngine->rootContext()->setContextProperty(
      "joystickManager", qgcApp()->toolbox()->joystickManager());
  qmlEngine->rootContext()->setContextProperty("debugMessageModel",
                                               AppMessages::getModel());
  qmlEngine->load(QUrl(QStringLiteral("qrc:/qml/MainRootWindow.qml")));
}

//-----------------------------------------------------------------------------
bool CustomPlugin::adjustSettingMetaData(const QString &settingsGroup,
                                         FactMetaData &metaData) {
  bool parentResult =
      QGCCorePlugin::adjustSettingMetaData(settingsGroup, metaData);
...
//-----------------------------------------------------------------------------
void CustomPlugin::paletteOverride(QString colorName,
                                   QGCPalette::PaletteColorInfo_t &colorInfo) {
  if (colorName == QStringLiteral("window")) {
    colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupEnabled] =
        QColor("#212529");
    colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupDisabled] =
        QColor("#212529");
    colorInfo[QGCPalette::Light][QGCPalette::ColorGroupEnabled] =
        QColor("#ffffff");
    colorInfo[QGCPalette::Light][QGCPalette::ColorGroupDisabled] =
        QColor("#f8f9fa");
